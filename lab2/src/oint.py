...
def handle_interpolation(req):
	...
	matrices = {}
		...
		if key == '1':
			tz = translation_matrix((0, 0, d+pos_change[0]))
			rz = rotation_matrix(th+pos_change[1], zaxis)
		if key == '2':
			tz = translation_matrix((0, 0, d))
			rz = rotation_matrix(th+pos_change[2], zaxis)
		if key == '3':
			tz = translation_matrix((0, 0, d))
			rz = rotation_matrix(th-  (1.57)/(int(freq*req.time)+1), zaxis)
